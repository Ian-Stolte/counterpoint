...
public class Player2Controller : MonoBehaviour
{
    ...
    public void Initialize(PlayerControls controls_)
    {
        ...
        controls.Player2.Move.performed += ctx => MovePressed(ctx.ReadValue<Vector2>());
        controls.Player2.Move.canceled += ctx => MovePressed(Vector2.zero);

        controls.Player2.Jump.performed += ctx => JumpPressed();
    }
